...
export function SuscripcionEmpleador() {
  ...
  const { plans, loading: plansLoading, error: plansError } = usePlans("EMPLOYER");
  ...
  const handleSelectPlan = async (plan: PricingPlan) => {
    ...
    try {
      ...
      const invoice = sessionResponse.invoiceId || sessionResponse.sessionId || `EMP-${userId}-${planId}-${Date.now()}`;
    ...
    <div className="w-full px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto flex flex-col gap-8 pb-12 animate-in fade-in zoom-in-95 duration-500">
      <div className="flex flex-col gap-2 mt-4 sm:mt-0">
        ...
        <p className="text-slate-500 max-w-2xl">
          {isActive 
            ? "Gestiona tu suscripción empresarial y revisa los beneficios activos de tu cuenta premium."
      ...
      <div className="w-full relative mt-4">
        ...
        {/* Estado de error de status (o si no es activo y hay error en planes) */}
