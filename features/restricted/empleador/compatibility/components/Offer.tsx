...
import { JobOfferDTO } from "../../jobOffer/types/jobOffer.types";
    ...
    return (
        ...
        <motion.div 
            ...
            className="lg:hidden group bg-white border border-gray-200 rounded-xl p-4 md:p-6 relative overflow-hidden flex flex-col"
        >
            <div className={`absolute top-0 left-0 right-0 h-1 transition-all duration-700 ${isActive ? 'bg-linear-to-r from-green-300 to-blue-500' : 'rounded-l-xl bg-linear-to-r from-yellow-200 to-orange-500'}`} />
                ...
                <div className="flex items-start gap-4 flex-1 min-w-0">
                    <div className="w-20 h-20 md:w-24 md:h-24 rounded-xl bg-gray-50 border border-gray-100 flex items-center justify-center shadow-inner shrink-0 overflow-hidden">
                        {imageUrl ? (
                            ...
                            <div className="w-full h-full bg-linear-to-br from-green-500 to-green-600 flex items-center justify-center text-white text-3xl font-bold">
                                {offer.title?.charAt(0).toUpperCase()}
                            </div>
                        )}
                    </div>
                    <div className="min-w-0 flex flex-col pt-1">
                        <h3 className="text-base md:text-lg font-bold text-gray-700 truncate leading-tight mb-1">{offer.title}</h3>
                        <div className="flex items-center gap-1 text-green-600 text-[11px] md:text-xs font-bold mb-2">
                        ...
                        <div className="hidden md:flex flex-wrap gap-2 mb-2">
                            <div className="px-2.5 py-1.5 bg-orange-50 text-orange-700 text-xs font-medium border border-orange-100 flex items-center gap-1.5 shadow-sm">
            ...
            <div className="flex justify-between items-center border-t pt-3 border-gray-200 mt-auto">
                <div className="flex flex-wrap gap-1.5 flex-1 pr-4">
                    {offer.requirements?.slice(0, 3).map((req: any, index: number) => (
                        <span key={index} className="bg-blue-50 border border-blue-200 text-blue-700 px-2 py-1 rounded-md text-[9px] md:text-[10px] font-bold uppercase tracking-wider shadow-sm">
        ...
        <motion.div 
            ...
            className="hidden lg:flex group bg-white p-6 gap-6 shadow-sm hover:shadow-md rounded-xl transition-all duration-300 relative overflow-hidden w-full"
        >
            <div className={`absolute left-0 top-0 bottom-0 w-3 transition-all duration-700 rounded-l-xl ${isActive ? 'bg-linear-to-r from-green-500 to-emerald-300' : 'bg-linear-to-b from-orange-500 to-amber-300'}`} />
                ...
                <div className="flex justify-between items-center border-t pt-2 border-gray-100">
                    <div className="flex justify-between">
                        ...
                        {offer.requirements?.slice(0, 4).map((req: any, index: number) => (
                            <span key={index} className="bg-blue-50 border border-blue-200 text-blue-700 px-3 py-1 rounded-lg text-xs font-medium  tracking-wider shadow-sm group-hover:border-green-100 transition-colors">
                            {typeof req === 'string' ? req : (req.requirementName || req.name || "Requisito")}
                            </span>
                        ))}
                        </div>
